"""Custom Weaviate Vector store index.
...
from src.components.vector_store.custom.weaviate.utils import (
    add_node,
    class_schema_exists,
...
class CustomWeaviateVectorStore(BasePydanticVectorStore):
    ...
    def from_params(
        ...
        **kwargs: Any,
        ...
        """Create CustomWeaviateVectorStore from config."""
        client_kwargs = client_kwargs or {}
    ...
    def query(self, query: VectorStoreQuery, **kwargs: Any) -> VectorStoreQueryResult:
        ...
        if query.mode == VectorStoreQueryMode.DEFAULT:
            ...
            logger.debug("Using Sparse Search")
            similarity_key = "score"

            if query.query_str:
                query_builder = query_builder.with_bm25(
                    query=query.query_str, properties=["text"]
                )
        ...
        for i, entry in enumerate(entries):
            if i < query.similarity_top_k:
                if query.mode == VectorStoreQueryMode.SPARSE:
                    similarities.append(entry["_additional"][similarity_key])
                else:
