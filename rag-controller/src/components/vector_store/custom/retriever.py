...
class HybridRetriever(BaseRetriever):
    def __init__(
        ...
        self.retrieval = index.as_retriever(
            similarity_top_k=self.similarity_top_k, vector_store_query_mode="hybrid"
        )
        super().__init__()

    def _retrieve(self, query_bundle: QueryBundle, **kwargs) -> List[NodeWithScore]:
        nodes = self.retrieval.retrieve(query_bundle, **kwargs)
        return nodes
