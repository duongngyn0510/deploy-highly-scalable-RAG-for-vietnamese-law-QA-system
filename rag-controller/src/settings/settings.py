...
from src.settings.settings_loader import load_active_settings
...
class VectorstoreSettings(BaseModel):
    database: Literal["chroma", "qdrant", "postgres", "weaviate"]
...
class VLLMSettings(BaseModel):
    ...
    temperature: float = Field(1.0, description="The temperature to use for sampling.")
...
class RerankSettings(BaseModel):
    ...
    mode: Literal["text_embeddings_inference", "model_api", "local"] = Field(
        "model_api", description="Mode to serve rerank model"
    )
    top_n: int = Field(
        3,
        description="This value controls the number of documents returned by the RAG pipeline after rerank",
    )


class ModelAPIRerank(BaseModel):
    model_api_endpoint: str


class TextEmbInfRerank(BaseModel):
    text_embeddings_inference_endpoint: str
    timeout: int = Field(60)


class LocalRerank(BaseModel):
    model: str = Field(
        "cross-encoder/ms-marco-MiniLM-L-2-v2",
        description="Rerank model to use. Limited to SentenceTransformer cross-encoder models.",
    )

class HybridRetrieverSettings(BaseModel):
    enabled: bool = Field(
        True,
        description="This value controls whether a custom retriever should be included in the RAG pipeline.",
    )
    similarity_top_k: int = Field(10)
    # version: Literal["v1", "v2"] = Field("v1", description="Hybrid retriever version")


# class HybridRetrieverV1Settings(BaseModel):
#     retrieval_top_k: int = Field(5)
#     dense_threshold: float = Field(0.35)
#     bm25_threshold: float = Field(8.0)


# class HybridRetrieverV2Settings(BaseModel):
#     retrieval_top_k: int = Field(5)
#     dense_threshold: float = Field(0.35)
#     bm25_threshold: float = Field(8.0)
#     alpha: float = Field(0.8)
...
class RagSettings(BaseModel):
    ...
    hybrid_retriever: HybridRetrieverSettings
...
class WeaviateSettings(BaseModel):
    weaviate_endpoint: str = Field("The server hosting the Weaviate vector database")
    api_key: str
    index_name: str
...
class TextEmbInfSettings(BaseModel):
    text_embeddings_inference_endpoint: str
    timeout: int
...
class Settings(BaseModel):
    ...
    text_embeddings_inference: TextEmbInfSettings
    ...
    model_api_rerank: ModelAPIRerank
    text_embeddings_inference_rerank: TextEmbInfRerank
    local_rerank: LocalRerank
...
print(unsafe_settings["local_rerank"])
