...
from src.paths import models_cache_path
...
class EmbeddingComponent:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        match embedding_mode:
            ...
            case "sagemaker":
                try:
                    from src.components.embedding.custom.sagemaker import (
                        SagemakerEmbedding,
                    )
                except ImportError as e:
                    raise ImportError(
            ...
            case "text_embeddings_inference":
                try:
                    ...
                    raise ImportError(
                        "TextEmbeddingsInference dependencies not found, please install or implement it"
                    ) from e

                text_embeddings_inference_settings = settings.text_embeddings_inference

                base_url = (
                    text_embeddings_inference_settings.text_embeddings_inference_endpoint
                )
                timeout = text_embeddings_inference_settings.timeout
